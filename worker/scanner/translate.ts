...
const VIOLATION_MAP: Record<string, ViolationTranslation> = {
  ...
  'duplicate-id': {
    ...
    description:
      'Two or more elements on this page share the same ID attribute. IDs must be unique — each ID should appear only once on a page.',
    fix: 'Make each ID unique across the entire page. If elements were duplicated by a template or loop, add a unique suffix (like "email-1", "email-2").',
  ...
  'bypass': {
    ...
    severity: 'serious',
    impact:
      'Keyboard-only users and screen reader users must tab through the entire navigation menu on every single page before reaching the main content.',
    description:
      'The page has no way to skip past the navigation to the main content. A "skip navigation" link allows users to jump straight to the content.',
    fix: 'Add a "Skip to main content" link as the very first element in the page body. It should link to the main content area using an anchor: <a href="#main-content" class="skip-link">Skip to main content</a>. Then add id="main-content" to your main content container.',
  ...
  'region': {
    ...
    description:
      'Some content on this page is not contained within a landmark region. Landmarks (like <header>, <nav>, <main>, <footer>) help screen reader users understand the layout.',
    fix: 'Wrap all page content in appropriate landmark elements. Use <header> for the site header, <nav> for navigation menus, <main> for the primary content, <aside> for sidebars, and <footer> for the footer.',
    wcagCriteria: ['1.3.1 Info and Relationships'],
  },
  'autocomplete-valid': {
    ...
    severity: 'moderate',
    impact:
      'Users who rely on browser autofill may get incorrect suggestions or no suggestions at all.',
    description:
      'A form field has an autocomplete attribute with an invalid or inappropriate value.',
    fix: 'Use the correct autocomplete value for each field type. Common values: "name" for full name, "email" for email, "tel" for phone, "street-address" for address, "postal-code" for ZIP code.',
  ...
  'link-in-text-block': {
    ...
    severity: 'serious',
    impact:
      'People with color blindness cannot tell which text is a link and which is not.',
    description:
      'A link within a block of text is only distinguished from surrounding text by color alone.',
    fix: 'Add an underline to links within text blocks, or provide another non-color visual indicator (like bold weight or an icon).',
  ...
  'tabindex': {
    ...
    severity: 'serious',
    impact:
      'Keyboard users will encounter a confusing or illogical tab order.',
    description:
      'An element has a tabindex value greater than 0, which forces it into a specific tab order position.',
    fix: 'Remove positive tabindex values and rely on the natural DOM order for tab navigation. Use tabindex="0" to make non-interactive elements focusable, and tabindex="-1" for programmatically focusable elements.',
  ...
  'focus-order-semantics': {
    ...
    severity: 'moderate',
    impact:
      'Keyboard and screen reader users may encounter focusable elements that are not announced with a meaningful role.',
    description:
      'An element that can receive keyboard focus does not have an appropriate semantic role.',
    fix: 'Use native HTML elements that have built-in semantic roles (like <button>, <a>, <input>) instead of generic elements like <div> or <span> with tabindex.',
    wcagCriteria: ['4.1.2 Name, Role, Value'],
  },
  'landmark-one-main': {
    ...
    severity: 'moderate',
    impact:
      'Screen reader users have no way to jump directly to the main content of the page.',
    description:
      'The page does not have a <main> element or an element with role="main".',
    fix: 'Wrap the primary content of the page in a <main> element. There should be exactly one <main> element per page.',
    wcagCriteria: ['1.3.1 Info and Relationships'],
  },
  'page-has-heading-one': {
    ...
    severity: 'moderate',
    impact:
      'Screen reader users often navigate to the H1 heading first to understand what the page is about.',
    description:
      'The page does not have a level-one heading (H1). Every page should have exactly one H1.',
    fix: 'Add one H1 heading near the top of the main content area that clearly describes the purpose of the page.',
    wcagCriteria: ['1.3.1 Info and Relationships'],
  },
  'image-redundant-alt': {
    ...
    severity: 'minor',
    impact:
      'Screen reader users will hear the same information twice.',
    description:
      'An image\'s alt text duplicates text that already appears adjacent to the image.',
    fix: 'Either change the alt text to provide different information, or set alt="" (empty) if the image is adequately described by its context.',
    wcagCriteria: ['1.1.1 Non-text Content'],
  },
  'meta-refresh': {
    ...
    description:
      'The page uses a meta refresh tag to automatically reload or redirect after a set time period.',
    fix: 'Remove the <meta http-equiv="refresh"> tag. If a redirect is needed, use a server-side redirect (HTTP 301/302) instead.',
  ...
  'video-caption': {
    ...
    severity: 'critical',
    impact:
      'Deaf and hard-of-hearing users cannot access the audio content of the video.',
    description:
      'A video element does not have captions (subtitles) provided.',
    fix: 'Add captions to the video using a <track> element with kind="captions". Captions should include all dialogue, speaker identification, and relevant sound effects.',
  ...
  'definition-list': {
    ...
    severity: 'moderate',
    impact:
      'Screen readers will not correctly announce the terms and definitions.',
    description:
      'A definition list (<dl>) contains elements other than <dt> (term) and <dd> (definition) as direct children.',
    fix: 'Ensure the <dl> element only contains <dt> and <dd> elements as direct children.',
    wcagCriteria: ['1.3.1 Info and Relationships'],
  },
  'dlitem': {
    ...
    severity: 'moderate',
    impact:
      'Screen readers will not associate this term or definition with a definition list.',
    description:
      'A <dt> or <dd> element exists outside of a <dl> container.',
    fix: 'Wrap <dt> and <dd> elements inside a <dl> container.',
    wcagCriteria: ['1.3.1 Info and Relationships'],
  },
  'scope-attr-valid': {
    ...
    severity: 'moderate',
    impact:
      'Screen readers may not correctly associate table headers with their data cells.',
    description:
      'A table header (<th>) uses a "scope" attribute with an invalid value.',
    fix: 'Correct the scope attribute to use one of the valid values: scope="col" for column headers, scope="row" for row headers.',
    wcagCriteria: ['1.3.1 Info and Relationships'],
  },
  'nested-interactive': {
    ...
    severity: 'critical',
    impact:
      'Screen readers and keyboard users may not be able to access the inner interactive element.',
    description:
      'An interactive element (like a button or link) is nested inside another interactive element.',
    fix: 'Restructure the HTML so interactive elements are not nested.',
    wcagCriteria: ['4.1.2 Name, Role, Value'],
  },
};
