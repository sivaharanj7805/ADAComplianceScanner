...
function classifyError(err: unknown): { code: ScanErrorCode; message: string } {
  ...
  if (msg.includes('net::ERR_NAME_NOT_RESOLVED') || msg.includes('getaddrinfo ENOTFOUND')) {
    return {
      code: 'DNS_FAILURE',
      message: "We couldn't reach this website. Please check the URL and make sure it's spelled correctly.",
    };
  }

  if (msg.includes('net::ERR_CERT') || msg.includes('SSL') || msg.includes('ssl') || msg.includes('CERT_')) {
    return {
      code: 'SSL_ERROR',
      message: "There's an SSL certificate issue with this site. The certificate may be expired or invalid. Try using http:// instead of https://.",
    };
  }

  if (msg.includes('net::ERR_CONNECTION_REFUSED') || msg.includes('ECONNREFUSED')) {
    return {
      code: 'CONNECTION_REFUSED',
      message: "We couldn't reach this website. The server may be down or blocking automated scans. Please try again later.",
    };
  }

  if (msg.includes('TimeoutError') || msg.includes('timeout') || msg.includes('Timeout') || msg.includes('Navigation timeout')) {
    return {
      code: 'TIMEOUT',
      message: 'This page took too long to load. Try again or scan a different page.',
    };
  }
  ...
  if (msg.includes('net::ERR_') || msg.includes('ECONNRESET')) {
    return {
      code: 'CONNECTION_REFUSED',
      message: "We couldn't reach this website. The server may be experiencing network issues. Please try again later.",
    };
  }
  ...
  if (msg.includes('crashed') || msg.includes('Target closed')) {
    return {
      code: 'PAGE_CRASH',
      message: 'The page caused the browser to crash during scanning. This usually happens with very large or resource-intensive pages. Try scanning a different page.',
    };
  }
...
export async function scanPage(url: string): Promise<PageScanOutcome> {
  ...
  try {
    ...
    try {
      ...
      if (navMsg.includes('timeout') || navMsg.includes('Timeout')) {
        try {
          ...
          return {
            success: false,
            error: { url, error: classified.message, errorCode: classified.code, timestamp: new Date().toISOString() },
          };
        }
      } else {
    ...
    if (httpStatus && httpStatus >= 400) {
      ...
      const errorCode: ScanErrorCode = (httpStatus === 401 || httpStatus === 403) ? 'AUTH_REQUIRED' : 'HTTP_ERROR';

      return {
        success: false,
        error: {
          url,
          error: statusMessages[httpStatus] ?? `This page returned an error (HTTP ${httpStatus}). Make sure the URL is correct and try again.`,
          errorCode,
          timestamp: new Date().toISOString(),
        },
      };
