...
//
// This is the competitive advantage of AccessAudit. Every translation is
// written so a town clerk, agency PM, or e-commerce store owner can read a
// violation report and understand:
//   1. What's wrong (in plain English)
//   2. Who is affected and how
//   3. What needs to happen to fix it
//
// We map every common axe-core rule ID to a full translation. For any rule
// NOT in this map, we generate a reasonable translation from axe metadata.
// Raw rule IDs are NEVER shown to users.
// ============================================================================
...
const VIOLATION_MAP: Record<string, ViolationTranslation> = {
  ...
  // ---------------------------------------------------------------------------
  // Additional common violations beyond the core 30
  // ---------------------------------------------------------------------------
  'autocomplete-valid': {
...
/**
 * Extract WCAG criteria references from axe-core tags.
 * axe-core tags look like: ["wcag2a", "wcag143", "cat.color"]
 * We want to extract the numbered criteria (e.g., "wcag143" → "1.4.3 Contrast (Minimum)")
 */
 ...
 * axe-core uses: minor, moderate, serious, critical (same as ours)
 */
...
/**
 * Generate a fallback plain-English translation from axe-core metadata.
 * Used when a violation rule ID is not in our pre-defined map.
 * This ensures users NEVER see raw rule IDs.
 */
function generateFallbackTranslation(violation: AxeViolation): ViolationTranslation {
  // Convert rule ID to a readable title: "aria-required-attr" → "Aria Required Attr"
  ...
  // Use axe-core's built-in help text as the description if available
...
/**
 * Translate an axe-core violation into plain English.
 *
 * This is the core of our competitive advantage. Every violation is translated
 * so that a non-technical person (town clerk, agency PM, store owner) can:
 *   1. Understand what is wrong
 *   2. Understand who is affected
 *   3. Understand what needs to happen to fix it
 *
 * For known rules (30+ of the most common), we use hand-written translations.
 * For unknown rules, we generate a reasonable translation from axe metadata.
 */
export function translateViolation(axeViolation: AxeViolation): TranslatedViolation {
  ...
  // Extract HTML snippets and CSS selectors from affected nodes
  ...
  // Determine WCAG criteria: prefer our curated list, fall back to tag extraction
  const wcagCriteria =
    translation.wcagCriteria.length > 0
      ? translation.wcagCriteria
      : extractWcagCriteria(axeViolation.tags);

  // For known rules, use our curated severity. For unknown, use axe-core's.
...
/**
 * Translate all violations from an axe-core scan result.
 * Returns violations sorted by severity (critical first, minor last).
 */
